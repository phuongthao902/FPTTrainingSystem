...
namespace TrainingFPTCo.Models.Queries
{
    public class CourseQuery
    {
            ...
            )
        {
            ...
            {
                string sql = "UPDATE [Course] SET [CategoryId] = @CategoryId, [Name] = @Name, [Description] = @Description, [Image] = @Image, [StartDate] = @StartDate, [EndDate] = @EndDate, [Status] = @Status, [UpdatedAt] = @UpdatedAt WHERE [Id] = @Id AND [DeletedAt] IS NULL";
                connection.Open();
        ...
        public bool DeleteCourseById(int id)
        {
            ...
            using (SqlConnection connection = Database.GetSqlConnection())
            {
                string sqlQuery = "UPDATE [Course] SET [DeletedAt] = @DeletedAt WHERE [Id] = @id";
                connection.Open();
        ...
        public CourseDetail GetDetailCourseById(int id)
        {
            ...
            using (SqlConnection connection = Database.GetSqlConnection())
            {
                string sql = "SELECT * FROM [Course] WHERE [Id] = @id AND [DeletedAt] IS NULL";
                connection.Open();
                SqlCommand cmd = new SqlCommand (sql, connection);
                cmd.Parameters.AddWithValue("@id", id);
        ...
        public List<CourseDetail> GetAllDataCourses()
        {
            ...
            using (SqlConnection connection = Database.GetSqlConnection())
            {
                string sql = "SELECT [co].*, CONVERT(VARCHAR(10), [co].[StartDate], 101) AS Start_Date,CONVERT(VARCHAR(10), [co].[EndDate], 101) AS End_Date, [ca].[Name] AS [CategoryName] FROM [Course] AS [co] INNER JOIN [Categories] AS [ca] ON [co].[CategoryId] = [ca].[Id] WHERE [co].[DeletedAt] IS NULL";
                connection.Open();
                SqlCommand cmd = new SqlCommand(sql, connection);
                using (SqlDataReader reader = cmd.ExecuteReader())
                {
        ...
        )
        {
            ...
            using (SqlConnection connection = Database.GetSqlConnection())
            {
                string sqlQuery = "INSERT INTO [Course] ([CategoryId], [Name], [Description],[Image], [StartDate], [EndDate], [Status], [CreatedAt]) VALUES  (@CategoryId, @Name, @Description, @Image, @StartDate, @EndDate, @Status, @CreatedAt); SELECT SCOPE_IDENTITY()";
                SqlCommand cmd = new SqlCommand( sqlQuery, connection );
                connection.Open();
                cmd.Parameters.AddWithValue("@CategoryId", categoryId);
                cmd.Parameters.AddWithValue("@Name", nameCourse);
